...
## 📋 작성일자
- **최초 작성**: 2025-11-07
- **검증 버전**: 2025-11-07 커밋 (0f72773) 기준
- **센서 매핑**: 하드코딩 방식 (addr+chip+pin 조합)
- **검증 상태**: ✅ 실제 하드웨어 환경에서 안정적 작동 확인 (전체 60개 센서)
...
2. handle_sensor_triggered() 호출
   ↓
3. addr+chip+pin → 센서 번호 하드코딩 매핑
   예: addr=0x26, Chip0, Pin1 → 센서 1 (S01)
   예: addr=0x23, Chip0, Pin1 → 센서 11 (M01)
   ↓
...
**센서 번호는 addr(칩 주소) + chip_idx + pin 조합으로 하드코딩되어 있습니다.**

```python
chip_addr_pin_to_sensor = {
    # addr=0x26, Chip0 → 교직원 (S01-S10): 센서 1-10
    ("0x26", 0, 1): 1,   # S01
    ("0x26", 0, 0): 2,   # S02
    # ...
    
    # addr=0x23, Chip0 → 남성 (M01-M10): 센서 11-20
    ("0x23", 0, 1): 11,  # M01
    ("0x23", 0, 2): 12,  # M02
    # ...
    
    # addr=0x25, Chip1 → 남성 (M11-M20): 센서 21-30
    ("0x25", 1, 0): 21,  # M11
    # ...
    
    # addr=0x26, Chip2 → 남성 (M21-M40 일부): 센서 31-50
    # addr=0x27, Chip3 → 남성 (M31-M40 일부) + 여성 (F01-F10): 센서 41-60
}
```

### 중요 포인트

1. **같은 Chip0이지만 addr로 구분**:
...
3. **절대로 공식을 사용하지 않음**:
   - ❌ `sensor_num = (chip_idx * 16) + pin + 1` (작동 안함)
   - ✅ 하드코딩된 매핑 테이블 사용 (실제 테스트로 검증됨)
